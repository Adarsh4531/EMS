...
const autoCheckOut = inngest.createFunction(
  ...
  async ({ event, step }) => {
   ...
   await step.sleepUntil("wait-until-9-hours", new Date(new Date().getTime() + 9 * 60 * 60 * 1000))
   ...
   await sendEmail({
    ...
    body: `
                <div style="max-width: 600px;">
                    ...
                    <p style="font-size: 18px; font-weight: bold; color: #007bff; margin: 8px 0;">${attendence?.checkIn?.toLocaleTimeString()}</p>
   ...
   //After 10 hrs, mark attendence as checked out with status "LATE"

   await step.sleepUntil("wait-until-1-hours", new Date(new Date().getTime() + 1 * 60 * 60 * 1000))
  ...
  async ({ event, step }) => {
   ...
   //Wait for 24 hrs
    ...
    if(leaveApplication?.status === "PENDING"){
      ...
      // Send reminder email to admin about the pending leave application
...
const attendenceReminderCron = inngest.createFunction(
   { id: "attendence-reminder-cron", triggers: [{ cron: "TZ=Asia/Kolkata 30 11 * * *" }] },//06:00 UTC = 11:30 AM IST
  async ({ step }) => {
    ...
    const checkedInIds = await step.run("get-checked-in-ids",async()=>{
      ...
      return attendences.map((a)=>a.employeeId.toString())
