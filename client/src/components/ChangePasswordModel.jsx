...
const ChangePasswordModel = ({open,onclose}) => {
  ...
  const handlesubmit = async (e)=>{
    ...
    const currentPassword = formData.get('currentPaasword')
    const newPassword = formData.get('newPaasword')
    ...
    <div onClick={onclose} className='fixed inset-0 z-50 flex items-center justify-center p-4'>
      ...
      <div className='relative bg-white rounded-2xl shadow-2xl w-full max-w-md animate-fade-in' onClick={(e)=> e.stopPropagation()} >
        ...
        <form className='p-6 space-y-5' onSubmit={handlesubmit}>
          {message.text && (
            <div className={`p-3 rounded-xl text-sm flex items-start gap-3 ${message.type === "sucess" ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : 'bg-rose-50 text-rose-700 border border-rose-200'}`}>
          ...
          <div>
            ...
            <input type="password" name="currentPaasword" required />
          </div>

           <div>
            <label className='block text-sm font-medium text-slate-700 mb-2'>New Password</label>
            <input type="password" name="newPaasword" required />
          </div>
